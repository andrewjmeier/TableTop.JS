...
SimpleView.prototype.drawBoard = function() {
    x_pos = 0;
    y_pos = 0;
    for (i = 0; i < this.game.spaces.length; i++) {
        tile = new PIXI.Graphics();
        tile.lineStyle(1, 0, 1);
        tile.drawRect(x_pos, y_pos, constants.canvasWidth / constants.canvasHeight)
        this.tileViews.push()
    }
...
SimpleView.prototype.drawTokens = function() {
    for (i = 0; i < this.game.players.length; i++) {
        this.updateToken(this.game.players[i].position, constants.colors[i]);
    }
}

SimpleView.prototype.updateToken = function(position, color) {
...
SimpleView.prototype.updateTokens = function() {
    for (i = 0; i < this.game.players.length; i++) {
        this.drawToken(this.game.players[i].position, constants.colors[i]);
    }
}
